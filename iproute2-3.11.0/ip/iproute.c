 ...
 *
 * Modified by: Ming Zhu, <zmvictorcruse@gmail.com>
 *
 */
...
#define PRTFL(fl,flname) if (flags&RTCF_##fl) { \
  ...
  first = 0; }
	...
	if (tb[RTA_DEFLECTION]) {
		struct rtnexthop *nh = RTA_DATA(tb[RTA_DEFLECTION]);
		int first = 0;

		len = RTA_PAYLOAD(tb[RTA_DEFLECTION]);
...
static int parse_nexthops(struct nlmsghdr *n, struct rtmsg *r,
			  int argc, char **argv, bool deflect)
{
	char buf[1024];
	...
	if (rta->rta_len > RTA_LENGTH(0)) {
        if (deflect)
		    addattr_l(n, 1024, RTA_DEFLECTION, RTA_DATA(rta), RTA_PAYLOAD(rta));
        else
...
static int iproute_modify(int cmd, unsigned flags, int argc, char **argv)
{
	...
	while (argc > 0) {
		if (strcmp(*argv, "src") == 0) {
			...
			nhs_ok = 1;
			break;
		} else if (strcmp(*argv, "deflect") == 0) {
            deflect = true;
            //printf ("deflect it is\n");
	...
	if (nhs_ok)
		parse_nexthops(&req.n, &req.r, argc, argv, deflect);
